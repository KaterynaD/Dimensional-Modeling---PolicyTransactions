CREATE OR REPLACE PROCEDURE control_db.sps.load_iceberg_fact_policytransaction()
...
/*----------------------DELETE EXISTEN IF ANY FOR THE SAME  TRANSACTION DATES--------------------------------------*/
  DELETE FROM mytest_db.POLICY_TRN_EXTTBL.fact_policytransaction f
  WHERE EXISTS( SELECT 1 FROM MYTEST_DB.POLICY_TRN_STAGING_EXTTBL.STG_PT_NEW_DATA  stg Where f.transactiondate = stg.transactiondate);
  
  num_deleted := SQLROWCOUNT;
  
/*---------------------------------------       INSERT NEW       -------------------------------------------------*/
INSERT INTO mytest_db.POLICY_TRN_EXTTBL.fact_policytransaction
(
/*-----------------------------------------------------------------------*/
...
--
left outer join mytest_db.POLICY_TRN_EXTTBL.dim_policy dim_policy
on stg.policy_uniqueid=dim_policy.policy_uniqueid
--
left outer join mytest_db.POLICY_TRN_EXTTBL.dim_coverage dim_coverage
on stg.coveragecd=dim_coverage.coveragecd
and stg.subline=dim_coverage.subline
and stg.asl=dim_coverage.asl
--
left outer join mytest_db.POLICY_TRN_EXTTBL.dim_limit dim_limit
on stg.limit1=dim_limit.limit1
and stg.limit2=dim_limit.limit2
--
left outer join mytest_db.POLICY_TRN_EXTTBL.dim_deductible dim_deductible
on stg.deductible1=dim_deductible.deductible1
and stg.deductible2=dim_deductible.deductible2
--
left outer join mytest_db.POLICY_TRN_EXTTBL.dim_driver dim_driver
...
--
left outer join mytest_db.POLICY_TRN_EXTTBL.dim_vehicle dim_vehicle
...
CREATE OR REPLACE PROCEDURE control_db.sps.load_iceberg_agg_table(month_id integer)
RETURNS VARCHAR NOT NULL
LANGUAGE SQL
AS
DECLARE
...
BEGIN
  ...
  rs := (
        ...
        from mytest_db.POLICY_TRN_EXTTBL.fact_policytransaction 
  ...
  FOR record IN rs DO
    ...
    delete from mytest_db.POLICY_TRN_EXTTBL.dummy_agg_table 
    ...
    insert into mytest_db.POLICY_TRN_EXTTBL.dummy_agg_table 
    (
     agg_id, 
     month_id,
     policy_id,
