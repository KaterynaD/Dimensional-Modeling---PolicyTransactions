--Snowflake Monitoring does not work for Iceberg tables 

CREATE OR REPLACE PROCEDURE control_db.sps.load_iceberg_testing_PK()
...
show primary keys in schema MYTEST_DB.POLICY_TRN_EXTTBL;
...
CREATE OR REPLACE PROCEDURE control_db.sps.load_iceberg_testing_FK()
RETURNS VARCHAR NOT NULL
LANGUAGE SQL
AS
DECLARE
...
show IMPORTED keys in schema MYTEST_DB.POLICY_TRN_EXTTBL;
...
CREATE OR REPLACE PROCEDURE control_db.sps.load_iceberg_testing()
RETURNS VARCHAR NOT NULL
LANGUAGE SQL
AS
DECLARE
...
from mytest_db.POLICY_TRN_STAGING_EXTTBL.STG_PT stg
)
, fact as 
(
select count(*) cnt  ,
