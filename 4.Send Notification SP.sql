CREATE OR REPLACE PROCEDURE control_db.sps.load_iceberg_complete_notification()
...
and table_schema='POLICY_TRN_EXTTBL'
...
order by  table_name
  ...
  FOR record IN rs DO
   ...
   query := 
   'with ll as (select max(loaddate) d, count(*) t from mytest_db.POLICY_TRN_EXTTBL.' || table_name || ')' ||
   'select  ''<tr> ' ||
   ' <td>'' || ''' || table_name  ||  ''' || ''</td> ' || 
   ' <td>'' || count(*)   || ''</td> ' || 
   ' <td>'' || ll.d       || ''</td> ' || 
   ' <td>'' || ll.t       || ''</td> ' || 
   ' </tr>'' as d ' ||
   ' from mytest_db.POLICY_TRN_EXTTBL.' || table_name || ' trg ' ||
   ' join ll ' ||
   'on 1=1 ' ||
