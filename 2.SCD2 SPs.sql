...
CREATE OR REPLACE PROCEDURE control_db.sps.load_iceberg_scd2_dim_driver()
RETURNS STRING
LANGUAGE SQL
AS
DECLARE
...
stg.maritalstatuscd,
stg.pointscharged
from MYTEST_DB.POLICY_TRN_STAGING_EXTTBL.stg_driver stg
)
...
dim.maritalstatuscd,
dim.pointscharged
from  mytest_db.POLICY_TRN_EXTTBL.dim_driver dim
...
update mytest_db.POLICY_TRN_EXTTBL.dim_driver t
...
CREATE OR REPLACE PROCEDURE control_db.sps.load_iceberg_scd2_dim_vehicle()
RETURNS STRING
LANGUAGE SQL
AS
DECLARE
...
from MYTEST_DB.POLICY_TRN_STAGING_EXTTBL.stg_vehicle stg
)
/*-- we need existing data from dim to compare with new add/not add with/without changes --*/
, existing_data as (
select
...
from  mytest_db.POLICY_TRN_EXTTBL.dim_vehicle dim
...
/*----------------Adjusting valid_todate - valid_fromdate in a case of backdated transactions*/
update mytest_db.POLICY_TRN_EXTTBL.dim_vehicle t
set t.valid_todate=ctes.valid_todate
from (
with changed_data as (
select
